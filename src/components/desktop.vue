<template>
  <div class="body overflow-hidden" :class="content !== 'blog' && content !== 'project' && content !== 'post' ? 'slide-' + windows[content].slide : ''" @wheel="scrollingWheel($event)">
    ...
    <post v-if="content === 'post'" :lang="lang" :id="postId" @goBlog="content = 'blog'" @goHome="content = 'main'" @nav="menuActive = !menuActive" @nextPost="postId++" />
  </div>
</template>

<script>
import FixWrapper from "./desktop/FixWrapper";
import Main from "./desktop/pages/main";
import Agency from "./desktop/pages/agency";
...
import project from "./desktop/projects/project/project";
import post from "./desktop/blog/post";
